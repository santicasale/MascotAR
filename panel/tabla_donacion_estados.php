...
$sql = "SELECT * FROM donacion_estado ORDER BY id_don_status ASC";
$res = $conn->query($sql);
...
<head>
  <meta charset="UTF-8">
  <title>Tabla-estado-de-donacion</title>
...
<section class="about">
  <div class="ver-admin-container">
    <h2>tabla de estados de donaciones</h2> 
    <?php if ($res->num_rows > 0): ?>
      <table border="1" cellpadding="10" cellspacing="0" style="margin:auto; background:white; border-collapse:collapse; width:90%;">
        ...
        <?php while ($row = $res->fetch_assoc()): ?>
            <tr>
                <td><?php echo $row['id_don_status']; ?></td>
                <td><?php echo htmlspecialchars($row['don_status']); ?></td>
                <td>
                    <a href="editar_donacion_estados.php?id=<?php echo $row['id_don_status'] ?>" class="btn-table btn-warning">Editar</a>
                    <a href="elimina_donacion_estados.php?id=<?php echo $row['id_don_status'] ?>" class="btn-table btn-danger">Eliminar</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </table>
